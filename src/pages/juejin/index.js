...
$(document).ready(function () {
    ...
    $.getJSON('/v1/gen_suid?src=juejin.im').then((gen_suid_response) => {
        if (gen_suid_response.m !== 'ok') {
            ...
            var suid = gen_suid_response.d;
            return $.getJSON(`/v1/get_recommended_entry?suid=${suid}&ab=welcome_3&src=web`);
        }
    }).done(get_recommended_entry_response => {
        recommendedData =  get_recommended_entry_response.d ? {...get_recommended_entry_response}
        : {
            d: []
        };
        ...
        console.log('get recommened entry failed: ' + err);
    });
